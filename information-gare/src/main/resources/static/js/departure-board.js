...
var audio = new Audio('../audio/asmr.wav');


DepartureBoard.prototype.setValue = function (value, latence) {


    audio.addEventListener('timeupdate', trucAudio, false);
    ...
    for (var r = 0, rl = this._letters.length; r < rl; r++) {
        ...
        for (var i = 0, l = this._letters[r].length; i < l; i++) {
            (function (r, i, l) {
                ...
                }, latence * r + 25 * i + Math.random() * 400);
            })(r, i);
        }
    }
};
...
DepartureBoard.Letter.prototype._tick = function () {
    ...
    if (this._index === this._stopAt) {
        audio.removeEventListener('timeupdate', trucAudio);
